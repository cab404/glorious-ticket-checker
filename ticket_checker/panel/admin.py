...
def download_stuff(modeladmin, request, queryset: QuerySet[Ticket]):
    ...
    for a in queryset:
        qr = qrmaker.make_qr_code_image(
            a.code,
            qr_code_options=qrmaker.QRCodeOptions(
                size="M",
...
def send_emails(modeladmin, request, queryset: QuerySet[Ticket]):
    ...
    def form_mail(ticket):
        mail = EmailMultiAlternatives(
            "Билет на кон",
            f"В этом письме ­— твой QR код для прохода на кон!",
            settings.EMAIL_HOST_USER,
            [ticket.email],
            connection=conn,
            alternatives=[
                (
                    render_to_string("panel/email.html", context={"ticket": ticket}),
                    "text/html",
                ),
            ],
        )
        mail.attach(
            "ticket.png",
            qrmaker.make_qr_code_image(
                ticket.code,
                qrmaker.QRCodeOptions(
                    size="M",
                    alignment_dark_color="#F00",
                    light_color="#FFFF",
                    image_format="png",
